// Initialize Firebase
var config = {
    apiKey: "AIzaSyBBy56mo40v0XLlIZr_e0aCpMIssEUa9n4",
    authDomain: "trainsched-e84bf.firebaseapp.com",
    databaseURL: "https://trainsched-e84bf.firebaseio.com",
    projectId: "trainsched-e84bf",
    storageBucket: "trainsched-e84bf.appspot.com",
    messagingSenderId: "335148173542"
  };
...
function findNextArrival(train) {
    ...
    var h0 = train.departure[1];
    var m0 = train.departure[2];
    var t0 = 60 * h0 + m0;
    ...
    var numTripsMade = Math.max(Math.floor((t1 - t0) / train.frequency), 0);
    
    // Find the arrival time
    var arrivalTime = t0 + (numTripsMade + 1) * train.frequency;
...
function displayTrain(train) {
    var info = findNextArrival(train);
...
function switchMode(mode) {
    if (mode === "add") {
        ...
        var h = myTrain.departure[1];
        var m = myTrain.departure[2];
...
function addTrain() {
    // Format the departure time
    var departure_string = $("#departure").val().trim();
...
$(document).ready(function() {
    ...
    var numSecondsLeft = 60 - (new Date()).getSeconds();
